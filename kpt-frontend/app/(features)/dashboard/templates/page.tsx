  ...
  const handleToggleShare = (id: string) => {
    setTemplates(
      templates.map(template =>
        template.id === id ? { ...template, isShared: !template.isShared } : template
      )
    );
  };
          ...
          {templates.map(template => (
            <div key={template.id} className='bg-white shadow rounded-lg overflow-hidden'>
              <div className='p-6'>
                  ...
                  <div>
                    ...
                    <ul className='mt-2 space-y-1'>
                      {template.keep.map((item, index) => (
                        <li key={index} className='text-sm text-gray-600 bg-indigo-50 p-2 rounded'>
                          {item}
                        </li>
                      ))}
                    </ul>
                ...
                <div className='mt-4 text-sm text-gray-500'>作成日: {template.createdAt}</div>
              </div>
            </div>
          ))}
        </div>
